...
export function ProjectDetailPage() {
  ...
  const [research, setResearch] = useState<ResearchWorkspace>({ requirements: [], sources: [], enrichment_running: false });
  ...
  const researchPanel = <div className="detail-stack"><Card className="workspace-panel research-summary" title="资料完整性" extra={<Button type="primary" icon={<CloudDownloadOutlined />} loading={researching} onClick={() => void runResearch()}>联网补全可信资料</Button>}><Typography.Paragraph type="secondary">系统会从权威监管、交易所、政府和国际机构检索公开资料。无法可靠补全的内容会保留在缺口清单，等待人工上传。</Typography.Paragraph><div className="evidence-grid">{research.requirements.map((item) => <div className={`evidence-card evidence-${item.status}`} key={item.id}><div className="evidence-card-head"><strong>{item.label}</strong><Tag color={item.status === "covered" ? "green" : item.status === "partial" ? "gold" : "red"}>{item.status === "covered" ? "已覆盖" : item.status === "partial" ? "部分覆盖" : "资料缺失"}</Tag></div><p>{item.reason}</p><span>建议资料：{item.suggested_document}</span></div>)}</div></Card><Card className="workspace-panel" title="公开研究来源"><List dataSource={research.sources} locale={{ emptyText: "尚未执行公开资料检索" }} renderItem={(source) => <List.Item actions={[<a href={source.url} target="_blank" rel="noreferrer">查看来源</a>]}><List.Item.Meta avatar={<span className="file-type-icon"><LinkOutlined /></span>} title={<Space>{source.title}<Tag color={source.status === "ingested" ? "green" : source.status === "failed" ? "red" : "gold"}>{source.status === "ingested" ? "已入库" : source.status === "review_required" ? "待人工复核" : source.status === "failed" ? "抓取失败" : "已发现"}</Tag></Space>} description={`${source.publisher} · ${source.evidence_category}${source.error ? ` · ${source.error}` : ""}`} /></List.Item>} /></Card></div>;
